  ...
  useEffect(() => {
    const fetchPublikasiDetail = async () => {
      try {
        ...
        // Convert API response to Publikasi format
      ...
      <Card className="w-full min-h-[500px] h-auto py-7 sm:p-5 mx-auto border-0 shadow-inner overflow-wrap">
        <CardHeader className="flex flex-col md:flex-row md:justify-between items-start gap-2">
          ...
          <div className="flex flex-col text-xs space-y-1 text-gray-700 text-left md:text-right">
            <p>
              Created At:{" "}
              {publikasi?.created_at ? new Date(publikasi.created_at).toLocaleString() : "N/A"}
            </p>
            <p>
              Updated At:{" "}
              {publikasi?.updated_at ? new Date(publikasi.updated_at).toLocaleString() : "N/A"}
            </p>
          </div>
        </CardHeader>
        ...
        <CardContent className="pb-10">
          <div className="space-y-4">
            <h1 className="text-xl font-bold">{publikasi.judul}</h1>
                ...
                <TableRow>
                  ...
                  <TableCell>
                    {isEditing ? (
                      ...
                      new Date(publikasi.tanggal).toLocaleDateString("id-ID", {
                        day: "2-digit",
                        month: "long",
                        year: "numeric"
                      })
